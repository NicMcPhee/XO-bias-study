...
plot are not statistically significant, two are: the differences between crossover bias 0 and crossover bias 0.50 and
...
crossover bias 0.75 and 1 ($p=0.43078$). Two other adjacent pairs have $p$-values slightly above 0.05: crossover bias
...
are statistically significant, with the exceptions being crossover bias 0 \emph{vs.} 0.50, and bias 0.50 and 1.0
...
bias 0 (standard subtree crossover) and all the other positive crossover bias values are statistically significant
($p<10^{-12}$), and all of the differences between crossover bias 0.25 and higher biases are significant ($p<0.007$);
however, none of the differences among 0.50, 0.75, and 1 are statistically significant. None of the differences for
...
substantially and significantly improves the hit rate, although all bias rates above 0.25 are very similar. For
...
\textbf{Waiting for treatment information on Figures~\ref{fig:Pagie1FitnessOverTime} and~\ref{fig:Pagie1SizeOverTime}. Figure~\ref{fig:Pagie1SizeOverTime} will eventually be removed, but the information is interesting, and I (Kirbie) would like as much information on it as possible before I remove it.}
...
quite minimal; \textbf{oddly, though, it does look like a crossover bias of 1 is worse by a little bit than everything
else at the end, which is weird}. Figure~\ref{fig:sineBiasFitnessVsGenT2E01P10K} shows fitness over time for the
non-bias-effective configuration with population size 10,240 (instead of 1,024 for the ``normal'' non-bias-effective
...
that is a really clean ``more bias is better'' demonstration (\textbf{Do we care?}) except for the fact that 0.75 and 1
are essentially the same. \textbf{Do we want to include both Figures~\ref{fig:sineBiasFitnessVsGenWeak}
