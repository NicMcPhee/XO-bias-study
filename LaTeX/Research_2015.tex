...
performance or had no impact - although the effectiveness is somewhat dependent on the problem, and significantly dependent on other parameter choices. Our results do, however, indicate the possibility that crossover bias may increase selection pressure and premature convergence - undesirable behavior, as it encourages a genetic programming run to arrive at a solution too quickly, in the process potentially excluding more accurate solutions for a more generalized one. \textbf{We have little to no text on this premature convergence point in the actual paper, and there's basically nothing on the broader question of asymmetry in the abstract, which I (Nic) think is part of what lifts the paper above just another random set of experiments.}
...
individual. This is in part due to the tendency of the root parent contributing more 
...
the root parent. In general, the results for reverse bias revealed that there was no significant difference from 
...
(\textbf{well? there is a paper that sort of describes it. search "us change problem" in gecco last year})

\textbf{Good catch. That's actually one of Lee's undergrad students, and he doesn't actually properly 
define the problem in the paper, unfortunately, so I wasn't sure whether to even bother linking to it. We
...
It should be noted that, for the ORDERTREE problem, a full sweep of the following parameters outlined in
Section~\ref{sec:Experiments}, with on exception: the population size for the ORDERTREE runs was limited to 1,024.
...
In addition to the parameters outlined in Section~\ref{sec:Experiments}, the following parameters were also implemented:

\begin{itemize}
	\item basic4 ($+, -, \times, \div$) and koza2 (complete - not discussed) function sets, and
	\item with and without Tarpeian bloat control.
\end{itemize}

\textbf{When we remove koza2 from Fig 9 then I think we'll be able to remove any mention of koza2, 
which means we can drop the bullet list and just make this a sentence.}
