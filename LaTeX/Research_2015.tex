...
Here we apply crossover bias to a variety of problems. In most cases we found that using crossover bias either improved
performance or had no impact -- although the effectiveness is somewhat dependent on the problem, and significantly dependent on other parameter choices. Our results do, however, indicate the possibility that crossover bias may increase selection pressure and premature convergence \textbf{-- undesirable behavior, as it encourages a genetic programming run to arrive at a solution too quickly, in the process potentially excluding more accurate solutions for a more generalized one}. \textbf{We have little to no text on this premature convergence point in the actual paper, and there's basically nothing on the broader question of asymmetry in the abstract, which I (Nic) think is part of what lifts the paper above just another random set of experiments.}
...
\textbf{This ``next set of runs'' refers to a figure that doesn't exist anymore. This paragraph and the next need to be combined into a single discussion of the Figure~\ref{fig:Ordertree_results_all_tournaments_Jan15}.}

However, in the next set of runs, using the full range of crossover bias values but limited to binary tournaments, we
observed a drop in fitness from bias 0.75 to bias 1.00, and 1.00 is actually slightly below that for 0.50 as well. All
