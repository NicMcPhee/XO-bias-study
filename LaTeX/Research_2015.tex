...
where the parent that contributes the root node is referred to as the \emph{root parent} and the other parent as
...
where it was observed that the fitness of the offspring tended to be better when the root parent had a better 
fitness than
the non-root parent.
...
results of GP runs, but that the results are heavily parameter dependent and, to a lesser degree 
problem dependent. In general, however, we find (Section~\ref{sec:Discussion}) that incorporating 
at least some crossover bias is, at least in these problems, usually either advantageous or neutral. 
