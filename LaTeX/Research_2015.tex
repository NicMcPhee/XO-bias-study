...
\textbf{Do we want to to include reverse bias (indicated with -1 in the plots) in all our analysis? If memory servers, 
nothing much interesting ever comes of it, and we've arguably got more data than we can analyze/discuss here, so 
I'm inclined to either remove it altogether, or just mention it once somewhere, maybe with one example in a plot. 
Thoughts?}

We did a full sweep of parameters for the K-Landscapes problem with both $K=2$ and $K=6$. \textbf{All I have 
added so far is the $K=6$ data, which is more interesting than the $K=2$ data because the problem is harder. Do we 
want to include both, or just forgot the $K=2$ results?}
...
combinations of parameter values. Increasing the amount of crossover bias appears to consistently improve the 
fitness of the results. All the differences are statistically significant ($p < 0.012$) except for the difference between 
reverse bias (-1) and no bias (0), and the difference between bias probability 0.75 and 1.0.
...
\begin{figure}
\centering
\includegraphics[width=0.45 \textwidth]{Plots/KLandscapes6_XO_bias_impact.pdf}
\caption{Impact of crossover bias on fitness for K-Landscapes problem, $K=6$.}
...
case than in the general case shown in Figure~\ref{fig:KLandscapes6_results}. Here all the differences are strongly 
...
\begin{figure}
\centering
\includegraphics[width=0.45 \textwidth]{Plots/KLandscapes6_XO_bias_strong_impact.pdf}
...
\textbf{Figures~\ref{fig:KLandscapes6_results_violin} and~\ref{fig:KLandscapes6_strong_results_violin} are variants 
of Figures~\ref{fig:KLandscapes6_results} and~\ref{fig:KLandscapes6_strong_results} using violin plots instead of box 
plots. I think I definitely prefer the violin plots in Fig~\ref{fig:KLandscapes6_strong_results} (the nice stair step plot), 
but I'm less certain about the other one. Thoughts?}

\textbf{Figure~\ref{fig:KLandscapes6_results_transformed_violin} is the same as Figure~\ref{fig:KLandscapes6_results_violin}, but with the scale transformed by squaring the values. It turns out that log didn't work; it just increased the compression up at the top because the values are between 0 and 1.}

\begin{figure}
\centering
\includegraphics[width=0.45 \textwidth]{Plots/KLandscapes6_XO_bias_impact_violin.pdf}
\caption{Impact of crossover bias on fitness for K-Landscapes problem, $K=6$.}
\label{fig:KLandscapes6_results_violin}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=0.45 \textwidth]{Plots/KLandscapes6_XO_bias_impact_transformed_violin.pdf}
\caption{Impact of crossover bias on fitness for K-Landscapes problem, $K=6$. Note the $y$-axis has been 
transformed to provide more detail in the upper end of the fitness range.}
\label{fig:KLandscapes6_results_transformed_violin}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=0.45 \textwidth]{Plots/KLandscapes6_XO_bias_strong_impact_violin.pdf}
\caption{Impact of crossover bias on fitness for K-Landscapes problem, $K=6$, restricted to binary tournament 
selection, no elitism, and population size 10,240.}
\label{fig:KLandscapes6_strong_results_violin}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.45 \textwidth]{Plots/Parent_fitnesses_sine.pdf}
\caption{Plot of the fitnesses of all individuals chosen as parents from some sine runs. \textbf{Explain this}.}
\label{fig:parentFitnessesSine}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=0.45 \textwidth]{Plots/Parent_diffs_sine.pdf}
\caption{Plot of the differences in parent fitnesses from some sine runs. \textbf{Explain this}.}
